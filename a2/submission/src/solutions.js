...
function trimChatMessages(chatCompletion, maxCharacters, trimDirection) {
  ...
  if (chatCompletion.messages.length === 0)
    throw new Error(
      'Trimming all messages is necessary to meet the maxCharacters limit, which is not allowed.'
    );
...
function formatChatCompletionToHTML(chatCompletion) {
  ...
  for (let i = 0; i < chatCompletion.messages.length; i++) {
    let UpperCaseFirstLetterInRole =
      chatCompletion.messages[i].role.charAt(0).toUpperCase() +
      chatCompletion.messages[i].role.slice(1);
    rows.push(
      `        <section class="${chatCompletion.messages[i].role}">${UpperCaseFirstLetterInRole}: ${chatCompletion.messages[i].content}</section>\n`
    );
  }
  rows.push('      </article>');
  return rows.join('');
...
function addUserName(chatData, defaultName) {
  ...
  chatData.messages.forEach((message) => {
    if (message.role === 'user' && !message.name) message.name = defaultName;
  });
}
...
function copySystemPrompts(source, target) {
  ...
  const systemPrompt = source.messages.filter((message) => {
  ...
  target.messages.unshift(...systemPrompt);
  return target;
}
