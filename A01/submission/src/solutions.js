...
function withTax(amount, code) {
  ...
  switch (code) {
    ...
    case 'QC':
      ...
      break;
    default:
      throw new Error('Invalid province/territory code.');
  }
  return amount * (1 + tax);
  // Replace this comment with your code...
  // In your solution, try to reduce duplicate code as much as possible for
  // codes with the same tax rate. HINT: consider a switch statement.
}
...
function long(count) {
  // Replace this comment with your code...
  // In your solution, you must use a for-loop
  if (count === 0 || !count) return 'long';
  let resString = 'l';
  for (let i = 0; i < count; resString += 'o', i++);
  resString += 'ng';
  return resString;
}
...
function mimeTypeFromFilename(filename) {
  // Replace this comment with your code...
  // HINT: you can use the `.endsWith()`, `.split()`, `.toLowerCase()` string methods
  let extension = filename.split('.')[1];
  if (!filename || !extension) return 'application/octet-stream';
